...
class ApplyCouponAdapter(var context: Context, var model: List<CouponsModel>) :
    ...
    override fun onBindViewHolder(holder: viewHolder, position: Int) {
        ...
        FirebaseFirestore.getInstance()
            ...
            .addSnapshotListener { querySnapshot: DocumentSnapshot?, e: FirebaseFirestoreException? ->
                querySnapshot?.let {
                    val userModel = it.toObject(CouponsModel::class.java)
                    if (model[position].id.equals(userModel?.id)) {
                        holder.usedCoupon.text = "Already Used"
                        holder.couponBg.setBackgroundTintList(context.getResources().getColorStateList(R.color.gray));
                        holder.couponBg.backgroundTintList = context.getColorStateList(R.color.gray)
                    }
        ...
        holder.itemView.setOnClickListener {
            if (!holder.usedCoupon.text.toString().equals("Already Used")) {
                MyCartAdapter.couponId.text = model[position].id
                if (model[position].title.equals("Discount")) {
                    var discountedPrice =
                        ((100 - model[position].price) * MyCartAdapter.couponOriginalPrice.text.toString()
                            .toInt()) / 100
                    MyCartAdapter.couponDiscountedPrice.text = discountedPrice.toString()
                    MyCartAdapter.couponApplyBtn.visibility = View.VISIBLE
                } else {
                    var flatOffPrice = MyCartAdapter.couponOriginalPrice.text.toString()
                        .toInt() - model[position].price
                    MyCartAdapter.couponDiscountedPrice.text = flatOffPrice.toString()
                    MyCartAdapter.couponApplyBtn.visibility = View.VISIBLE
